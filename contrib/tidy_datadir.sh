...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Cosanta database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Cosanta datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Cosanta datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Cosanta 0.7 datadir."
    ;;
  3)
    echo "Detected Cosanta pre-0.8 datadir."
    ;;
  4)
    echo "Detected Cosanta 0.8 datadir."
    ;;
esac
