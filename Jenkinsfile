...
// which are part of the Cosanta repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="cosanta-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /cosanta-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "cosanta/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from cosanta/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'cosanta/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from cosanta/develop"
            }
          }

          if (hasCache) {
            sh "cd /cosanta-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /cosanta-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /cosanta-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /cosanta-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /cosanta-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /cosanta-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /cosanta-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
